...
class TestSessionCRUD:
    ...
    def test_list_sessions(self, client):
        client.post("/api/chat/sessions", json={"user_id": "user1", "title": "A"})
        client.post("/api/chat/sessions", json={"user_id": "user1", "title": "B"})
        resp = client.get("/api/chat/sessions", params={"user_id": "user1"})
        assert resp.status_code == 200
        sessions = resp.json()["sessions"]
        assert len(sessions) == 2

    def test_get_session_messages(self, client):
        create_resp = client.post("/api/chat/sessions", json={"user_id": "u1"})
    ...
    def test_get_nonexistent_session(self, client):
        resp = client.get("/api/chat/sessions/no-such-id")
